...
void initValue(float *values, int *exponents, float *output, float *gold, unsigned int N)
{
  ...
  {
    // random input values
    values[i] = -1.f + 4.f * static_cast<float>(rand()) / RAND_MAX; // [-1 ,3]
...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
  ...
  __cs149_vec_float one = _cs149_vset_float(1.0f);
  __cs149_vec_float threshold = _cs149_vset_float(9.999999f);
  __cs149_mask maskAll, yZeroMask, yOneMask, gtThresholdMask;
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH)
  {
    ...
    _cs149_veq_int(yZeroMask, y, zero, maskAll); // y == 0 ?
    _cs149_vmove_float(result, one, yZeroMask);  // out = 1

    yOneMask = _cs149_mask_not(yZeroMask);
    int cnt = _cs149_cntbits(yOneMask);

    // if has y != 0
    while (cnt > 0)
    {
      _cs149_vmult_float(result, result, x, yOneMask);
      _cs149_vsub_int(y, y, oneInt, yOneMask);
      _cs149_veq_int(yZeroMask, y, zero, maskAll);
      yOneMask = _cs149_mask_not(yZeroMask);
      cnt = _cs149_cntbits(yOneMask);
    }

    // if result > threshold
    _cs149_vgt_float(gtThresholdMask, result, threshold, maskAll);
    _cs149_vmove_float(result, threshold, gtThresholdMask);
